...
class CharService {
    ...
    var characterList: List<Character> = listOf(
        ...
        Character("Roy", "Marron", "Marron", arrayOf<String>("Gafas"), arrayOf<String>("Obscura"),21), // 21
        Character("Scott", "Marron", "Verde", arrayOf<String>(), arrayOf<String>("Obscura", "Bigote"),22), // 22
    ...
    fun turn(property: String, value: String) {
        ...
        if (boolCompare) {
            ...
            println("andamios en el false jerjer")
            for (c in this.characterList) {
                if (!c.removed) {
                    when (property) {
